...
4. batch_no and lot are usually the same — set both when only one label appears.
5. Read rotated/vertical text carefully. Preserve leading zeros (00603113 not 70603113).
...
BATCH / LOT: Batch, BNO, B.NO, LOT, Lot, Lot No, LOT NO., Lote, (10), [LOT] box symbol.
...
EXP DATE — hourglass icon (ISO 15223) OR text: EXP, EXP.DATE, CAD, (17)
  ...
  When three 8-digit dates appear with symbols: [LOT]=batch, factory=mfg, hourglass=exp.
...
def _repair_record(item: dict[str, Any]) -> dict[str, Any]:
    ...
    batch = str(item.get("batch_no") or item.get("lot") or "").strip()
    ...
    if batch and _looks_like_date(batch) and item.get("exp_date") and not item.get("mfg_date"):
        batch_date = normalize_date(batch)
        exp_date = normalize_date(str(item["exp_date"]))
        if batch_date and exp_date and batch_date < exp_date:
            item["mfg_date"] = batch_date
...
def _record_from_dict(item: dict[str, Any]) -> MedicineRecord:
    ...
    batch = item.get("batch_no") or item.get("batch")
    lot = item.get("lot") or batch
    filled = sum(
        1
    ...
    return MedicineRecord(
        ...
        source_fields={
            ...
            if item.get(k) or (k in ("batch_no", "lot") and batch)
        },
    )


def _call_groq(client: Any, image_bytes: bytes, mime: str, user_text: str) -> dict[str, Any]:
...
def extract_with_groq(image_bytes: bytes, mime: str = "image/jpeg") -> list[MedicineRecord]:
    ...
    payload = _call_groq(client, image_bytes, mime, user_text)
    records = _payload_to_records(payload)

    if _needs_retry(records):
        retry_payload = _call_groq(client, image_bytes, mime, RETRY_PROMPT)
